...
while tw < leng                             %Each iteration is 1 frame
    ...
    if ~isempty(dots)
        Screen('drawdots',window,dots,dotsize,colour.grey);
    end
    ...
    % %MODIFIED BY HB% adjusted the punch rectangle
    % %draw square in upper right for MEG DC 2016-02
    % baseRect = [0 0 100 100];
    % windowsize=Screen('Rect',0); %i'm sure we had it somewhere else...but let's retrieve it again
    % positionedRect = CenterRectOnPointd(baseRect, windowsize(3)-baseRect(3)/2, baseRect(4)/2);
